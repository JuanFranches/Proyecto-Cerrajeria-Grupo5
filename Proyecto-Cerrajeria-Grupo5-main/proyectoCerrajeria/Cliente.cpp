...
void Cliente::Cargar(){
    // ID con validación numérica + control de repetido en archivo
    while (true) {
        cout << "ID cliente: ";
        if (!(cin >> idCliente)) {
        ...
        // limpio el \n que queda en el buffer si todo salió bien
        cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

        // --- CONTROL DE ID UNICO EN clientes.dat ---
        bool repetido = false;
        ...
        // si llegó hasta acá: ID válido numéricamente y no repetido
        break;
    }
